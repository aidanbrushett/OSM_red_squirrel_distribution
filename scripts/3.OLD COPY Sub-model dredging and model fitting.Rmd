...
## 6.1. Odds ratio plots for the conditional model:
...
## 6.2. Plot conditional effects for a fixed effect of interest (no interaction)
...
## 6.3. Plot conditional effects for a fixed effect of interest **with** an interaction
...
models_rank_table <- models_rank %>%
  ...
  summarize(formula = paste(predictor, collapse = " + ")) %>%
  
  mutate(formula = ifelse(formula == "intercept", "1", str_replace(formula, "intercept \\+ ", "")),
  ...
  select("Hypothesis" = "model", "Covariates" = "formula", df, AICc, "ΔAICc" = "delta", weight)
