...
# covariate data 
covs <- read_csv("data/raw/OSM_covariates_grouped_2021_2022_2023.csv")
...
```{r separate dataframes for each modelling step}

core_data <- data_long %>%
   dplyr::select(
    array, site, red_squirrel, absent_red_squirrel, lc_grassland, lc_coniferous, lc_mixed,
    lc_broadleaf, lc_shrub)

dist_data <- data_long %>%
 dplyr::select(
    array, site, red_squirrel, absent_red_squirrel, harvest, seismic, linear, 
    wells, roads, osm_industrial)
