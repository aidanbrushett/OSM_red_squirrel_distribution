...
```{r}
data <- read_csv("./data/processed/all_arrays_proportional_presence_weekly_clean.csv",
                 col_types = cols(array = col_character(),
                                  site = col_character(),
                                  .default = col_number())) %>%
...
landscape_covs <- data %>%
  ...
  mutate(coyote_abundance = coyote / weeks_active,
         lynx_abundance = lynx / weeks_active) %>%
...
```{r}
write_csv(data, "./data/processed/all_arrays_proportional_presence_weekly_with_covariates.csv")
```
