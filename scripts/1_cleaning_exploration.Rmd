...
config <- read_csv("data/raw/OSM_configuration_simplified_2021_2022_2023.csv")

# rename the buffer column
config <- config %>%
  rename("buff_dist" = "buffer")
...
data_long <- config %>%
  left_join(data_long, by = c("site", "buff_dist"))

data_long <- data_long %>%
  dplyr::select(site, buff_dist, array, red_squirrel, absent_red_squirrel, harvest,
          seismic, roads, linear, wells, lc_grassland, lc_coniferous, 
         lc_broadleaf, lc_mixed, lc_shrub, osm_industrial, natural_cohesion,          natural_ed, anthropogenic_ed,
         natural_tca, anthropogenic_tca)

data_long$anthropogenic_tca[is.na(data_long$anthropogenic_tca)] <- 0
data_long$anthropogenic_ed[is.na(data_long$anthropogenic_ed)] <- 0

write.csv(data_long, "data/processed/analysis_df_long.csv")
```
...
data_wide <- data_long %>%
  ...
  dplyr::select(site, buff_dist, red_squirrel, absent_red_squirrel, harvest, seismic, 
        roads, linear, wells, lc_grassland, lc_coniferous, 
         lc_broadleaf, lc_mixed, lc_shrub, osm_industrial, natural_cohesion, 
         natural_ed, anthropogenic_ed,
         natural_tca, anthropogenic_tca) %>% 
...
write.csv(data_wide, "data/processed/analysis_df_wide.csv")
```
...
squirrel <- buffer_frames %>% 
  ...
  purrr::map(
    ~.x %>% 
      ...
      dplyr::select(red_squirrel, everything()) %>%
      dplyr::select(absent_red_squirrel, everything()))
...
squirrel$'2000 meter buffer' %>%
      dplyr::select(harvest:linear) %>% 
...
config_cor <- data_long[data_long$buff_dist == 250, ]


config_cor %>%
      dplyr::select(harvest:anthropogenic_tca) %>% 
