...
fig_scale <- bind_rows(natural_scale, 
  ...
  ggplot(., aes(x = buffer, y = weight, color = submodel)) + 
    ...
    scale_color_manual(values =c("goldenrod2","darkblue","forestgreen"),
...
ggsave("./figures/redsquirrel_neg_binomial_submodel_scale_modelweight.png", width = 4, height = 3, dpi = 600)
...
Store them manually here before we move to the next step.

```{r}
# Let's specify these values manually. This means we don't need to re-run this every time we open a new R session. 
nat_buffer <- 100
comp_buffer <- 4250
config_buffer <- 2250
```
...
data_final_scaled <- bind_cols(
  ...
  select(-landscape_ed, -landscape_tca, -landscape_cai_mn)
...
xxx <- data_final_corr %>% 
  filter(var1 != "cfi_site_with_harvest" & var2 != "cfi_site_with_harvest",
         var1 != "cfi_site_with_vegedges" & var2 != "cfi_site_with_vegedges",
         var1 != "nonanthro_cai_mn" & var2 != "nonanthro_cai_mn",
         var1 != "nonanthro_tca" & var2 != "nonanthro_tca") %>%
  ...
  select(-PrettyName) %>%
  drop_na() %>%
  filter(abs(corr)>0.50)

writexl::write_xlsx(xxx, "./tables/final_dataset_high_correlation.xlsx")
```
...
```{r}
  m_edgeXcfi_noranef <- glmmTMB(squirrel ~ 
...
anova(m_edgeXcfi_noranef, m_edgeXcfi)
```
...
## 7.2. Rate ratio plots for the conditional model:
...
ggsave(rate_fig_top, file = "./figures/top_model_neg_binomial_rate_ratios.png", width = 5, height = 3.5)
```


## 7.3. Rate ratio plots for the second-place model
...
ggsave(odds_fig_second, file= "./figures/second_place_model_neg_binomial_rate_ratios.png", width = 5, height = 3.5)
```


## 7.4. Export the combined top and second-ranking model figures in one!

```{r}
ggpubr::ggarrange(rate_fig_top, rate_fig_second, 
                  ncol = 2, 
                  nrow = 1,
                  align = "hv",
...
ggsave("./figures/rate_ratio_top_model_second_model.png", width = 7, height = 4.2, dpi = 700)
```
...
ggsave(f_decid, file = "./figures/top_model_neg_binomial_broadleaf.png", dpi=400, width = 6, height = 4)
```
...
# Save the figure
ggsave(f_conifer, file = "./figures/top_model_neg_binomial_conifer.png", dpi = 400, width = 6, height = 4)
```
...
# Save to file
ggsave(f_mixedwood, file = "./figures/top_model_neg_binomial_mixedwood.png", dpi = 400, width = 6, height = 4)
```
...
# Save the plot
ggsave(f_fire, file = "./figures/top_model_neg_binomial_fire.png", dpi = 400, width = 6, height = 4)
...
f_cfi <- ggeffects::ggpredict(top_model, terms = c("cfi_site [all]")) %>%
  ...
  ggplot(., aes(x = x, y = predicted)) +
    ...
    # Axis labels
    labs(x = "Cumulative Anthro. \nFootprint [2250 m]", 
         y = NULL) +
      
    # Remove padding on left of x-axis
    scale_x_continuous(expand = c(0, 0), limits = c(0, 0.302)) +
...
# Save the plot
ggsave(f_cfi, file = "./figures/top_model_neg_binomial_cfi.png", dpi = 400, width = 6, height = 4)
...
# Save the plot
ggsave(f_edge, file = "./figures/top_model_neg_binomial_edge.png", dpi = 400, width = 6, height = 4)
...
## 10.2. For the second place model
