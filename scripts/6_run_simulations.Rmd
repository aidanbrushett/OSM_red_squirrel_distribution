...
```{r}
true_vals_fixef <- enframe(fixef(m_edgeXcfi)$cond, name = "term", value = "value")

true_vals_ranef <- enframe(VarCorr(m_edgeXcfi)$cond, name = "term", value = "value") %>%
...
sim_coefficient_plots <- purrr::map(true_vals$term, ~{
  ...
  sim_master_results_df %>%
    ...
    ggplot(., aes(x = value)) + 
      ...
      geom_density(fill = ifelse(str_detect(label, "Random")==TRUE, "darkblue", "darkred"), alpha = 0.3) + 
    
      geom_vline(xintercept = true_val, 
                 linetype = "dashed", 
                 color = ifelse(str_detect(label, "Random")==TRUE, "darkblue", "darkred"), 
...
ggsave("./figures/top_model_simulations_coefficient_density_gaussian_sampler.png", width =9, height = 8)
```
...
ggsave("./figures/top_model_simulations_coefficient_density_uniform_sampler.png", width = 9, height = 8)
```
...
ggsave("./figures/top_model_simulations_coefficient_density_reduced_sample_size.png", width = 9, height = 8)
```

Let's look at some summary statistics for our empirical samples. Do they match the simulations?

```{r}
true_vals <- enframe(fixef(m_edgeXcfi)$cond, name = "term", value = "mean_model")
true_se <- enframe(summary(m_edgeXcfi)$coefficients$cond[, "Std. Error"], name = "term", value = "se_model")
