...
dets <- read_csv("data_squirrel/all_arrays_proportional_presence_weekly_clean.csv")
...
# covariate data 
covs <- read_csv("data_squirrel/OSM_covariates_grouped_2021_2022.csv")
...
```{r long format analysis dataframe for step AIC}

# join the covs and the dets because we want each buffer represented as a row not a separate column
long_data <- dets %>%
  left_join(covs, by = "site")

unique(long_data$buff_dist)
```


Create a list with all the covariates for each scale. This code was taken from Marissa's OSM analysis. 
...
# Formatting for step selection
Combine seismic and seismic 3D
Combine pipeline and transmission line
...
# combine the seismic and seismic 3D and pipe and transmission lines
