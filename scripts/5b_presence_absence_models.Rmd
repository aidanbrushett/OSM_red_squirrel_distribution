...
m_edgeXcfi_nonat = as.formula(paste("cbind(n_present, n_absent) ~ nonanthro_ed + cfi_site + nonanthro_ed*cfi_site +",   random_effects))
)
```
...
writexl::write_xlsx(model_compare, "./tables/edgeXcfi_natural.xlsx")
```
...
```{r}
m_edgeXcfi_noranef <- glmmTMB(cbind(n_present, n_absent) ~ 
...
m_edgeXcfi_noranef2 <- glmmTMB(cbind(n_present, n_absent) ~ 
...
summary(m_edgeXcfi_noranef)
summary(m_edgeXcfi_noranef2)

performance::check_overdispersion(m_edgeXcfi_noranef)

anova(m_edgeXcfi_noranef, m_edgeXcfi_noranef2)
```
...
models_rank_table <- models_rank %>%
  ...
  mutate(PrettyName = case_when(
    predictor == "cfi_site:nonanthro_ed" ~ "Edge Density × Cumulative Disturbance",
    TRUE ~ PrettyName)) %>%
...
fixed_effects <- broom.mixed::tidy(top_model, effects = "fixed", conf.int = TRUE) %>%
  ...
  mutate(Covariate = case_when(
    Term == "nonanthro_ed:cfi_site" ~ "Edge Density × Cumulative Disturbance",   # Human-readable label for interaction
